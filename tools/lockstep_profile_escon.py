...
with McpSession(rom='/home/chad/supermn-snes/build/interp.sfc',mesen=NEXEN,port=7524,boot_wait=6.0,socket_timeout=300.0) as m:
    ...
    w16(0xAC,AC); w16(0x071A,1); w16(0x0718,0)   # ESCAPES ON + streaming ON
    ...
    w16(0x410000,0x0100,'snesMemory'); w16(0x410002,0x0100,'snesMemory')  # hold Right (keep gameplay active)
    runf(180)                                 # longer: several ticks to get a representative interpreted count
    nb=r16(0x0718)
    ...
    print(">>> top GENUINELY-UNCOVERED regions (escapes ON, deterministic-ish):",flush=True)
