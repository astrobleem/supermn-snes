...
with McpSession(rom='/home/chad/supermn-snes/build/interp.sfc',mesen=NEXEN,port=int(os.environ.get('PORT','7542')),boot_wait=6.0,socket_timeout=300.0) as m:
    ...
    h_in=m.add_exec_hook(ENTRY, cpu_type='Sa1'); h_out=m.add_exec_hook(EXITA, cpu_type='Sa1')
    w16(0x0712,0); w16(0x0714,1)
    r=m.run_until(max_frames=10, hook_handle=h_rff)          # $3A92 release wake
    c0=cyc()
    w16(0x0710,0x0818); w16(0x0716,0)                        # tick-end trap
    spans=[]
    while True:
        r=m.run_until(max_frames=30, hook_handle=h_in)
        if (r or {}).get('reason')!='hookFired':
            break                                             # no more entries (tick likely ended)
        if r16(0x0712): break
        ci=cyc()
        ...
        spans.append(cyc()-ci)
        if r16(0x0712): break
        if len(spans)>2000: print("!! runaway",flush=True); break
    ended=r16(0x0712)
    c1=cyc()
    print(">>> tick ended=%s  fires=%d  RESIDENCY=%d cyc  (min=%s max=%s)"%(
        bool(ended),len(spans),sum(spans),min(spans) if spans else '-',max(spans) if spans else '-'),flush=True)
    print(">>> spans:",spans[:40],flush=True)
