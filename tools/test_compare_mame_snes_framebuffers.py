...
ARCHIVE = Path("/home/chad/supermn-snes-artifacts/build-archive-20260813")


def retained_fixture(relative: str) -> Path:
    candidates = (ROOT / "build" / relative, ARCHIVE / relative)
    return next((path for path in candidates if path.is_file()), candidates[0])


MAME = retained_fixture(
    "playtest-investigation-20260725/mame-one-credit-oracle-v2/one-credit-prompt.png"
)
SNES = retained_fixture(
    "validate-fresh-one-credit-prompt-isolated-a976-mesen211-v1/"
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="mame-snes-frame-test-") as raw:
        ...
        assert report.is_file(), run.stderr or run.stdout
