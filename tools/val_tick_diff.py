...
with McpSession(rom='/home/chad/supermn-snes/build/interp.sfc',mesen=NEXEN,port=7515,boot_wait=6.0,socket_timeout=300.0) as m:
    ...
    def one_tick(gate):
        ...
        return ok,w40,w41,R
    ok0,o40,o41,R0=one_tick(0)   # OFF (interp reference)
    ok1,n40,n41,R1=one_tick(1)   # ON  (escapes)
    print("GAME_TICK return R=$%06X ; OFF trapped=%s ON trapped=%s"%(R0,ok0,ok1),flush=True)
    d40=sum(1 for i in range(0x10000) if o40[i]!=n40[i])
    d41=sum(1 for i in range(0x8000) if o41[i]!=n41[i])
    print("ON-vs-OFF  $40 work-RAM diff=%d ; $41 video-shadow diff=%d"%(d40,d41),flush=True)
    if d40:
        ds=[i for i in range(0x10000) if o40[i]!=n40[i]]
        print("  first $40 diffs: %s"%["$F0%04X(off=%02X on=%02X)"%(i,o40[i],n40[i]) for i in ds[:12]],flush=True)
    print(">>>", "GREEN -- escapes bit-exact vs interp over one tick" if (ok0 and ok1 and d40==0 and d41==0)
          else "RED -- $40diff=%d $41diff=%d (froze off=%s on=%s)"%(d40,d41,ok0,ok1),flush=True)
