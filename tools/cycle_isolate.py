...
DRV=0xFE00; DRVBANK=0x00F0; BRA=DRV+6
...
drv = bytes([0x4E,0xB9,(TGT>>24)&0xFF,(TGT>>16)&0xFF,(TGT>>8)&0xFF,TGT&0xFF, 0x60,0xFE])  # jsr $00TGT ; bra.s self
with McpSession(rom='/home/chad/supermn-snes/build/interp.sfc',mesen=NEXEN,port=7537,boot_wait=6.0,socket_timeout=300.0) as m:
    ...
    for k in range(N):
        ...
        c0=cyc()
        w16(0x0714,0); w16(0x0712,0); w16(0x0710,BRA); w16(0x0716,DRVBANK)   # arm exit trap at bra-self
        hit=False
        for _ in range(40):
            runf(2)
