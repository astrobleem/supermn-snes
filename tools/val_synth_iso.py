...
ISO=os.environ['SUPERMN_ISO']
regs=open(ISO+'/entry_regs.bin','rb').read(); wramA=open(ISO+'/entry_wram.bin','rb').read()
assert len(wramA)==0x10000,'need 64KB entry'
xr=open(ISO+'/exit_regs.bin','rb').read(); xw=open(ISO+'/exit_wram.bin','rb').read()
...
with McpSession(rom='/home/chad/supermn-snes/build/interp.sfc',mesen=NEXEN,port=7465,boot_wait=6.0,socket_timeout=300.0) as m:
    ...
    def inject(hook):
        ...
        w16(0x0718,0xFFF8)
        for o in range(0,len(wramA),0x4000): wh(0x400000+o,wramA[o:o+0x4000].hex(),'snesMemory')
    ...
    # OFF (interp) vs MAME exit: regs (D0-D7,A0-A6) + work RAM, excluding scratch + our pushed return
    def be32(d,o): return (d[o]<<24)|(d[o+1]<<16)|(d[o+2]<<8)|d[o+3]
    mD=[be32(xr,i*4) for i in range(8)]; mA=[be32(xr,(8+i)*4) for i in range(7)]+[be32(xr,15*4)]
    mreg=[RN[k] for k in range(16) if rd(orf,k)!=( (mD+mA)[k] & 0xFFFFFFFF )]
    mw40=sum(1 for i in range(0x10000) if i not in ex and o40[i]!=xw[i])
    print("[vs MAME exit] regdiff=%s ; $40diff(excl scratch)=%d"%(mreg or 'NONE',mw40),flush=True)
    onoff = (not rdiff and bd40==0 and bd41==0 and nf and of)
    print(">>>", "GREEN — escape $%06X == interp (ON/OFF) ; interp==MAME=%s"%(TARGET, 'YES' if (not mreg and mw40==0) else 'partial') if onoff else "RED",flush=True)
