...
#   Production gates on (match the shipped tick).
import sys, os
...
TD=sys.argv[1]; PC1=int(sys.argv[2],16); PC2=int(sys.argv[3],16); POKE=len(sys.argv)>4 and sys.argv[4]=='1'
...
print("triple %s  PC1=$%06X PC2=$%06X  POKE(hle-off)=%s"%(TD,PC1,PC2,POKE),flush=True)
with McpSession(rom='/home/chad/supermn-snes/build/interp.sfc',mesen=NEXEN,port=int(os.environ.get('PORT','7542')),boot_wait=6.0,socket_timeout=300.0) as m:
    ...
    r3=m.run_until(max_frames=900, hook_handle=h_spin); c2=cyc(); i2=instr()
    print("trap2 reason=%s cyc=%s instr=%d"%((r3 or {}).get('reason'),c2,i2),flush=True)
    ok=all((x or {}).get('reason')=='hookFired' for x in (r1,r2,r3))
    print(">>> SPAN %06X->%06X = %s cyc  (interp-instr = %d)"%(PC1,PC2,(c2-c1) if ok else 'N/A',i2-i1),flush=True)
