#!/usr/bin/env python3
# HOOKTEST: confirm the native scheduler switch-OUT (entry_swo @ $92F86E) actually FIRES during a real
# GAME_TICK (SA-1 exec-hook count, NOT $07xx memory counters which the game overwrites). Inject
# ce4trip64, run ONE tick, report the fire count. Expect ~21/tick (one per task yield).
import sys, os
...
ENTRY=int(os.environ.get('ENTRY','0x92F86E'),0)
...
with McpSession(rom='/home/chad/supermn-snes/build/interp.sfc',mesen=NEXEN,port=7541,boot_wait=6.0,socket_timeout=300.0) as m:
    ...
    H=m.add_exec_hook(ENTRY, cpu_type='Sa1')
    # step PAST current $3A92, run ONE full tick to the next $3A92, counting entry_swo fires
    w16(0x0712,0); w16(0x0710,0); w16(0x0714,1); runf(1); w16(0x0714,0)
    fires=0
    for _ in range(80):
        w16(0x0710,0x3A92); w16(0x0716,0)
        r=m.run_until(max_frames=4, hook_handle=H)
        if r.get('reason')=='hookFired':
            fires+=1; continue
        if r16(0x0712): break
    print(">>> entry_swo @ $%06X fired %d times in one GAME_TICK (expect ~21; 0 => trampoline misrouted)"%(ENTRY,fires))
