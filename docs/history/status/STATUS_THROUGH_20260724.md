...
> **Historical status snapshot, superseded as an authority.** Preserve the dated
> evidence below, but use [`docs/current/STATUS.md`](../../current/STATUS.md) for every
> present-tense verdict, number, candidate, and next action.

Last updated: July 24, 2026. Per-area detail lives in the linked docs.
For recovery provenance, see [RECOVERY.md](../recovery/RECOVERY.md).
...
> formal performance remain open. See [RECOVERY.md](../recovery/RECOVERY.md) R15 and
> [the focused handoff](../handoffs/V135_IRAM_FREEZE_HUD_AUDIO_20260724.md).
...
> open. See [RECOVERY.md](../recovery/RECOVERY.md) R14 and
> [the focused handoff](../handoffs/V134_STAGE2_VERTICAL_SCROLL_20260724.md).
...
> [RECOVERY.md](../recovery/RECOVERY.md) R13 and
> [the focused handoff](../handoffs/V133_TITLE_ATTRACT_BOOT_20260723.md).
...
> and formal performance gates. See [RECOVERY.md](../recovery/RECOVERY.md) R12 and
> [the focused handoff](../handoffs/V132_TITLE_CRATE_RIGHT_EDGE_20260723.md).
...
> [RECOVERY.md](../recovery/RECOVERY.md) R11 and
> [the focused handoff](../handoffs/V130_SECOND_PLAYTEST_20260723.md).
...
> latest formal performance evidence. See [RECOVERY.md](../recovery/RECOVERY.md) R10 and
> [the focused handoff](../handoffs/FIRST_WALL_OCTAVE_AUDIO_AND_BOOT_20260723.md).
...
> repairs; the renderer-conservation failure remains current. See [RECOVERY.md](../recovery/RECOVERY.md) R9 and
> [the focused Mesen handoff](../handoffs/MESEN211_PLAYTEST_REGRESSIONS_20260723.md).
...
> [RECOVERY.md](../recovery/RECOVERY.md) R8 and
> [the focused handoff](../handoffs/CHARGED_SHOT_FREEZE_20260723.md).
...
> [CONFESSION.md](../recovery/CONFESSION.md) and [RECOVERY.md](../recovery/RECOVERY.md) R7.

> ## HISTORICAL R6 PERFORMANCE RESULT — July 22, 2026
>
> [CONFESSION.md](../recovery/CONFESSION.md) and [RECOVERY.md](../recovery/RECOVERY.md) R6 supersede the old performance
...
> sound-command mailbox → TAD triggers). Steering: docs/history/performance/PROFILE_CAMPAIGN.md.
...
> (`tools/measure_render_wram.py`). Memory `render-to-wram-pt21`; docs/history/performance/PROFILE_CAMPAIGN.md §pt.21.
...
>
> **STANDING DECISION (the realtime goal, deferred to the user — see docs/history/campaigns/MAIN_PLANNING_HANDOFF.md pt.11):**
...
> correctness fix, not the 24×-closer). See [docs/history/campaigns/MAIN_PLANNING_HANDOFF.md](../campaigns/MAIN_PLANNING_HANDOFF.md) top
...
> .l flag bug FIXED properly (97d5049, guard tools/val_branch32.py). See docs/history/campaigns/MAIN_PLANNING_HANDOFF.md pt.10.
...
> See docs/history/campaigns/MAIN_PLANNING_HANDOFF.md pt.7 + memory `transpiler-32bit-flag-bug`.
...
> (dbra-fallthrough CCR; hand-patched, proper fix TODO). See docs/history/campaigns/MAIN_PLANNING_HANDOFF.md pt.6 block.
...
> [docs/history/campaigns/MAIN_PLANNING_HANDOFF.md](../campaigns/MAIN_PLANNING_HANDOFF.md) + memory `scheduler-switchin-shipped`.
...
> start at task #10). See the [docs/history/campaigns/MAIN_PLANNING_HANDOFF.md](../campaigns/MAIN_PLANNING_HANDOFF.md) "IMMEDIATE NEXT ACTION".

> **UPDATE 2026-06-30 — read [docs/history/campaigns/MAIN_PLANNING_HANDOFF.md](../campaigns/MAIN_PLANNING_HANDOFF.md) for the
...
- **Phase A (SA-1) DONE** and **Phase B (hybrid native-escape) DONE**: a PC-hook
  ...
  `interp.pasm`. See `docs/history/designs/TRANSPILER_DESIGN.md` §D5.
...
|---|---|---|
| **Graphics pipeline** | ✅ validated on real SNES PPU vs MAME | `docs/toolchain/GRAPHICS_PALETTE_EVIDENCE.md` |
| **Transpiler design (D1–D4)** | ✅ settled | `docs/history/designs/TRANSPILER_DESIGN.md` |
| **Transpiler spike (gate G2)** | ✅ GREEN — 2 functions differentially verified | `SPIKE_RESULT.md` |
| **68K interpreter** | ✅ **BIT-EXACT vs MAME** on busy attract + active gameplay (lock-step diff; 4 opcode bugs fixed). Runs on the **SA-1**. Correctness gates **opsweep 782/782 + optest 154/154** (`tools/opsweep.py` op×EA grid + `tools/optest.py` per-opcode vs MAME — both SA-1-correct). Clears the **C-Chip boot handshake** (replay, not emulation). | `docs/history/experiments/INTERPRETER_BRINGUP.md`, `lockstep-harness-progress` memory |
| **Phase A — SA-1** | ✅ **DONE** — cart runs on SA-1 (work RAM in BW-RAM `$40`, shadow `$41`, dual-CPU video). | `sa1-bringup` memory |
| **Phase B — native-escape hook** | ✅ **DONE** — PC-hook routes hooked 68K calls (jsr.l / jsr(An) / bsr) to native 65816; `$412` RNG native, bit-identical. Profiler + save-state + speedup harness. Foundation hardened (leak fixed, `leaf_check.py`, FOUNDATION CONTRACT). | `docs/history/designs/TRANSPILER_DESIGN.md` §D5 |
| **Transpiler TOOL (automated)** | ✅ **BUILT + validated** — `tools/transpile.py` emits native escapes from 68K functions; reproduces the hand oracles bit-exact. Call-bridge (non-leaf) + `--video` (shadow stores). | `docs/history/plans/TRANSPILER_TOOL_SCOPE.md`, `transpiler-tool` memory |
...
| **Disassembly coverage (gate G1)** | ⬆ in progress — reliable pipeline + full playthrough | `docs/toolchain/TRACE_COVERAGE.md` |
...
banks/frame → 8 OBJ palettes suffice, no quantization. See `docs/toolchain/GRAPHICS_PALETTE_EVIDENCE.md`.

## Transpiler — design settled + spike green (G2)
- **Design (`docs/history/designs/TRANSPILER_DESIGN.md`)**: D1 carry/branch lowering (carry inverted on
...
- Peony recursive descent from these seeds → **35,047 blocks** (vs 483 baseline);
  ...
  classified. See `docs/toolchain/TRACE_COVERAGE.md`.
...
**[ROADMAP.md](../plans/ROADMAP.md)**. In short, in priority order:
