# docs/history/campaigns/MAIN_PLANNING_HANDOFF.md

> **Historical planning handoff.** It preserves the pre-v135 campaign and its
> corrections; it is not a current task list. Use
> [current status](../../current/STATUS.md) and
> [release blockers](../../current/RELEASE_BLOCKERS.md).
...
> `AGENTS.md`, and the newest section of `docs/history/performance/PROFILE_CAMPAIGN.md`.
...
docs/history/performance/PROFILE_CAMPAIGN.md §pt.21.) HARNESS LESSON (cost the session, don't relearn): the NAT
...
Steering doc: docs/history/performance/PROFILE_CAMPAIGN.md (bottom sections, newest last). Triples/NAT backup:
...
> `docs/history/designs/OBJECT_PROCESSOR_CAMPAIGN_20260703.md` §"A3 WIDEN" + the CP1/A2 sections of `docs/history/performance/PROFILE_CAMPAIGN.md`.
...
> (b) task-body long tail, (c) codegen efficiency (`legacy memory: workram-lever-bounded` bounds it). Use commit
...
> **⚠ Found TWO GENERAL transpiler bugs** (memory `legacy memory: transpiler-32bit-flag-bug`): `emit_signed_cmp`
...
> needs each escape's gate re-validated). See memory `legacy memory: transpiler-32bit-flag-bug`.
...
>   `jsr(a0)` blitter bridges + the c9a6 bridge (exactly `legacy memory: cycle-budget-realtime-gap`'s low-value
...
> is expensive+risky to escape cleanly and (per `legacy memory: cycle-budget-realtime-gap`) bridge-dominated /
...
> lever — but `legacy memory: workram-lever-bounded` pins EA-specialization at ≤20%, ~1.2×, not the ~5.7× realtime
...
> (root-cause). Memories `legacy memory: fetch-chokepoint-rts-escape` (breakthrough + fix + rollout),
> `legacy memory: rts-class-dispatch-nonfunctional` (partially superseded). Build GREEN (working tree, uncommitted).
...
> (`legacy memory: new-freeze-camera-gating`). Getting that triple is the explicit precondition for committing a
...
> `legacy memory: fetch-chokepoint-rts-escape` + `/tmp/supermn-scratch/chokegen/RESULTS.md`.
...
work is the transpiler codegen rewrite. See memory `legacy memory: cycle-budget-realtime-gap` (the authoritative
...
`legacy memory: fetch-chokepoint-rts-escape`, `legacy memory: rts-class-dispatch-nonfunctional` — the latter partially superseded.)
...
(memory `legacy memory: scheduler-context-switch-lever`.)
...
- **But ESC=0 lockstep = DIFF~44** (baseline GREEN) — a real but unpinned **integration** divergence
  ...
  `docs/history/handoffs/scheduler_switchout_wip.md`.
...
- `capture_at_pc` (MAME) has prefetch-skewed SP for stack-frame handlers like `$0532` (memory
  `legacy memory: mame-capture-precision`).
...
This is the only track that closes the 24× realtime gap. (memory `legacy memory: cycle-budget-realtime-gap`.)
...
- **FOUND the biggest lever:** scheduler = ~30% of the tick, pure coroutine context-switch plumbing
  (§2). Built `sched_trace.py`. Memory `legacy memory: scheduler-context-switch-lever`.
...
- Repo GREEN, escape not in the build; switch-OUT code preserved in `docs/history/handoffs/`.
