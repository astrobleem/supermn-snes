...
> **Historical roadmap.** Its experiments, rankings, and target changes are preserved,
> but it is not a current work queue. Use
> [release blockers](../../current/RELEASE_BLOCKERS.md) and the
> [Gigandes bring-up plan](../../gigandes/BRINGUP.md).

Last updated: July 4, 2026. Companion to [STATUS.md](../status/STATUS_THROUGH_20260724.md) (current state)
and [docs/history/plans/PORTING_PLAYBOOK_20260625.md](PORTING_PLAYBOOK_20260625.md) (the reusable recipe).
...
> resume its ranked escape list. [RECOVERY.md](../recovery/RECOVERY.md) R5 is the active roadmap: attribute the
...
> [docs/history/campaigns/MAIN_PLANNING_HANDOFF.md](../campaigns/MAIN_PLANNING_HANDOFF.md) (pt.20 banner) is authoritative for the
> current task; docs/history/performance/PROFILE_CAMPAIGN.md is the measurement ledger.

> **UPDATE 2026-06-30 — [docs/history/campaigns/MAIN_PLANNING_HANDOFF.md](../campaigns/MAIN_PLANNING_HANDOFF.md) is authoritative for the
...
its steady-state idle loop at PC `$0818`. See [docs/history/experiments/INTERPRETER_BRINGUP.md](../experiments/INTERPRETER_BRINGUP.md).
...
1. **Video write capture → SNES PPU.** Intercept the game's writes to the X1-001/
   ...
   (palette decode + the two draw paths — see [docs/toolchain/GRAPHICS_PALETTE_EVIDENCE.md](../../toolchain/GRAPHICS_PALETTE_EVIDENCE.md)).
...
3. **Inputs.** Feed controller state through the C-Chip input mailbox
   (already designed — see [CCHIP_BOOT_HANDSHAKE.md](../../current/CCHIP_BOOT_HANDSHAKE.md)).
...
- Push G1 from the reliable 10.2% confirmed-code floor toward the ≥85% code/data
  ...
  [docs/toolchain/TRACE_COVERAGE.md](../../toolchain/TRACE_COVERAGE.md).
- Produce the **G4 endianness manifest** (which data regions are byte-swapped) —
  policy is already set in D3 ([docs/history/designs/TRANSPILER_DESIGN.md](../designs/TRANSPILER_DESIGN.md)).
...
[CONVERTSOUND.md](../../toolchain/SOUND_CONVERSION_REFERENCE.md) and [SOUNDHARDWARE.md](../forensics/SOUND_HARDWARE_SURVEY.md).
...
## Acceptance gates (from [RISK_TRANSPILER.md](../risks/TRANSPILER.md))
