...
> **Historical initial plan.** Dates, architecture state, coverage, and “current”
> labels below are preserved as archaeology. Use
> [current status](../../current/STATUS.md) for present truth.

> **RECOVERY OVERRIDE — July 12, 2026:** this is the original project-plan snapshot, not current
> status. Start at [`RECOVERY.md`](../recovery/RECOVERY.md). The canonical build runs at 1.3237 game-fps
> post-arm; performance architecture is the active gate.

> **Current status: see [`STATUS.md`](../status/STATUS_THROUGH_20260724.md).** Snapshot (June 17, 2026):
> graphics path validated on real SNES PPU (`docs/toolchain/GRAPHICS_PALETTE_EVIDENCE.md`); transpiler
> design settled (`docs/history/designs/TRANSPILER_DESIGN.md`) and gate **G2 green** — two 68K leaves
...
> resolved, climbing via a full playthrough trace (`docs/toolchain/TRACE_COVERAGE.md`). Bulk
...
- [~] Disassemble 68K game code — trace-driven CDL pipeline (gate G1); 10.2%
      confirmed code, 779 jump tables resolved, climbing (`docs/toolchain/TRACE_COVERAGE.md`)
- [~] Write 68K→PASM transpiler — design settled (D1–D4, `docs/history/designs/TRANSPILER_DESIGN.md`);
