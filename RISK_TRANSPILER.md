...
> **The four "settle now" decisions (H1–H4) are settled in `TRANSPILER_DESIGN.md`**
...
> — see `TRANSPILER_DESIGN.md` §D4 for the corrected, ground-truth addresses.
...
- **G1 — Coverage** ≥85% executed code separated from data, via MAME-trace CDL.
  ...
  classified. See `COVERAGE_G1.md`. (The old "14%" was unreliable linear-sweep.)
...
- **G4 — Endianness manifest** exists and every converted table is on it.
  ⬜ NOT STARTED (policy settled in `TRANSPILER_DESIGN.md` §D3).
...
truth-validated address map now lives in **`TRANSPILER_DESIGN.md` §D4** — edit it
there, not here.
